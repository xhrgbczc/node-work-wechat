...
import { weEncrypt, weDecrypt, weGetSignature } from '@wecom/crypto';
...
WorkWechat.prototype = {
  ...
  encrypt (xmlMsg) {
    ...
    const ciphered = weEncrypt(this.options.encodingAESKey, xmlMsg, corpid);
  ...
  decrypt (echostr) {
    ...
    const { message, id } = weDecrypt(this.options.encodingAESKey, echostr);
  ...
  getSignature (token, timestamp, nonce, echostr) {
    ...
    return weGetSignature(token, timestamp, nonce, echostr);
  },
